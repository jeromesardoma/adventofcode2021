input = "gfeabcd adecfb gcedb cef efgdc decabg cbfg edfga fdegcb cf | fc faedg fdage fec
...
signals, segment_sets = [], []
consoles.each do |c|
  signal, segment_set = c
  signals << signal
  segment_sets << segment_set
end

easy_segment_count = 0
segment_sets.each do |ss|
  ss.split.each do |segment|
    easy_segment_count += 1 if [2, 3, 4, 7].include?(segment.length)
  end
end

p easy_segment_count

# part 2

=begin
  
1. find the segments common between "1" and "7": the remaining third segment 
must be the topmost segment
2. the segments that make up "9" = the topmost seg + segments of "4" + bottom seg
--> bottom seg = "9" segs - "4" segs
3. delete common segs from 0 and 9
--> lower left = seg remaining in 0
--> middle = seg remaining in 9
4. with above, upper right = unassigned char in "2"
5. with above, lower right = unassigned char in "1"
6. with above, upper left = 8 - all solved chars above
  
=end

def translate(signal_set)
  t, ul, ur, m, ll, lr, b = Array.new(7, nil)
  one_signal = signal_set.select { |s| s.length == 2 }.first
  seven_signal = signal_set.select { |s| s.length == 3 }.first
  t = seven_signal.delete(one_signal)

  four_signal = signal_set.select { |s| s.length == 4 }.first

  six_long_signals = signal_set.select { |s| s.length == 6 }
  nine_signal = six_long_signals.select { |s| (s - (t + four_signal)).length == 1 ) }
  # zero_signal = six_long_signals.select { |s| s != nine_signal }.first
  # six_signal = six_long_signals.select { |s| s! != nine_signal || s != zero_signal }.first
  b = nine_signal.delete(four_signal + t)

  ll = 'abcdefg'.delete(t + b + four_signal)
  
  # m = 'abcdefg'.delete(zero_signal)
  # ul = four_signal.delete(m + one_signal)

  five_long_signals = signal_set.select { |s| s.length == 5 }
  
  


  { (t + ul + ur + ll + lr + b).sort: 0,
    (ur + lr).sort: 1
    (t + ur + m + ll + b).sort: 2,
    (t + ur + m + lr + b).sort: 3,
    (ul + ur + m + lr).sort: 4,
    (t + ul + m + lr + b).sort: 5, 
    (t + ul + m + ll + lr + b).sort: 6,
    (t + ur + lr).sort: 7,
    (t + ul + ur + m + ll + lr + b).sort: 8,
    (t + ul + ur + m + lr + b).sort: 9
  }
end
